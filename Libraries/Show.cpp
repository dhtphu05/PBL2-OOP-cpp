...
Show::Show(string ID_Show, string date, string startTime, Movie* movie, DoubleLinkedList<ShowSeat> seatLayout, Screen* screen){
...
DoubleLinkedList<Show> Show::loadShowFromFile(string fileName){
    ...
    while (std::getline(file, line)) {
        ...
        if (line.rfind("ShowID:", 0) == 0) {
            ...
            currentShow->ID_Show = line.substr(8);
        } else if (line.rfind("Date:", 0) == 0) {
            currentShow->date = line.substr(6);
        } else if (line.rfind("StartTime:", 0) == 0) {
            currentShow->startTime = line.substr(11);
        } else if (line.rfind("EndTime:", 0) == 0) {
            currentShow->endTime = line.substr(9);
...
void Show::saveShowToFile(string fileName){
    ofstream file(fileName);
    Node<Show> *current = this->shows.begin();
    while(current!=nullptr){
        file << "ShowID: " << current->data.getID_Show() << endl;
        file << "Date: " << current->data.date << endl;
        file << "StartTime: " << current->data.startTime << endl;
        file << "EndTime: " << current->data.endTime << endl;
        //file << "Screen: " << current->data.screen.getID_Screen() << endl;
        Node<ShowSeat> *currentSeat = current->data.seats.begin();
        while(currentSeat!=nullptr){
            file << "Seat: " << currentSeat->data.getSeatRow() << " " << currentSeat->data.getSeatColumn() << " " << currentSeat->data.getType() << " " << currentSeat->data.getPrice() << " " << currentSeat->data.getIsBooked() << endl;
            currentSeat = currentSeat->next;
        }
        current = current->next;
    }
}
DoubleLinkedList<Show> shows; // Move this line outside the class

int main(){
    DoubleLinkedList<Show> shows = Show::loadShowFromFile("../Databases/Show.txt");
    shows.displayShowInfo();
    return 0;
